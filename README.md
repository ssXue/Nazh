...
- 工程看板、默认项目和示例载荷仍来自前端 demo data，尚未形成真正的项目保存、版本化、导入导出、迁移与回滚能力。
...
如果目标只是本地 PoC 演示，当前主链路已经可用；如果目标是“可直接投产的工业平台”，优先级应当按下面几个阶段推进，而不是继续只补 demo 节点。
...
- 项目持久化与版本管理：替换静态 `BOARD_LIBRARY` / demo AST，支持项目保存、导入导出、版本快照、迁移、回滚和环境差异配置。
...
- 数据与配置治理：为项目、连接、节点模板和运行参数建立 schema 版本、兼容策略和配置校验链路。
- 可靠性验证：增加协议集成测试、硬件在环测试、长稳压测、故障注入和升级回归测试，而不只是当前的单机单流程测试。

### P2：平台化

- 设备 / 连接 / 项目注册中心：从“本地桌面工作区”演进到可管理多项目、多设备、多现场的资产模型。
- 发布与变更管理：支持审批、灰度、回滚、配置差异对比和跨环境发布，而不是只在单机桌面直接部署。
