...
核心抽象：
- `ScriptNodeBase` — 持有 `AST` / `Scope` / `Engine` / `id` / `max_operations` / 可选 `Arc<dyn AiService>`
...
3. **`code` 节点例外**：它可以通过 `ai_complete()` 发 AI 请求——这是**内建的、显式的**副作用。由 `NazhScriptPackage` 注入，`AiService` 由 `ai` crate 提供。这也是为什么 `code` 节点不标 `PURE`。
...
- 允许：`nazh-core`、`ai`（需要 `AiService` 来实现 `ai_complete()`）、`rhai`、`serde_json`、`tracing`
- 禁止：`connections`、`nodes-*`、协议 crate

本 crate **允许**依赖 `ai`——这是 ADR-0019 关注的依赖关系。未来若 `ai` 的 `AiService` trait 上移到 Ring 0，本 crate 就可以只依赖 Ring 0。
...
- **（待）ADR-0019** AI 能力依赖反转——若落地，本 crate 依赖路径会简化
