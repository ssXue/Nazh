...
核心抽象：
- `ScriptNodeBase` — 持有 `AST` / `Scope` / `Engine` / `id` / `max_operations`
...
- `NazhScriptPackage` — 自定义 Rhai 包，提供 `rand()`、`now_ms()`、`is_blank()`、`from_json()`、`to_json()`、位操作/进制转换（`bit_get`/`bit_set`/`to_hex`/`from_hex`/`to_bin`）、数值工具（`round`/`clamp`/`scale`）、时间格式化（`format_ts`/`duration_ms`）等通用 helper
...
2. **脚本拿不到 `DataStore` 也拿不到 `ConnectionManager`**，但默认环境不是纯函数环境：`rand()` / `now_ms()` 非确定，`vars.set()` / `vars.cas()` 可写工作流变量。因此复用 `ScriptNodeBase` 的节点不能仅因”不直接访问硬件”就声明 `PURE`。
3. **~~`ai_complete()` 已随 RFC-0005 移除~~**。AI 调用已前移到前端（Vercel AI SDK），Rust 侧不再注入 AI helper。
...
- 允许：`nazh-core`、`rhai`、`chrono`、`fastrand`、`serde_json`、`tokio`
- 禁止：`ai`、`connections`、`nodes-*`、协议 crate

RFC-0005 已实施（2026-05-12）：AI HTTP 调用全部前移到前端，本 crate 不再持有 `AiService` 引用，`ai_complete()` 已移除。
...
- **ADR-0019** AI 能力依赖反转 — **已由 RFC-0005 取代**（2026-05-12），AI HTTP 调用全部前移到前端，`ai_complete()` 已移除
