...
- **AI 共享类型**：`AiToolCall` / `AiToolDefinition` / `AiToolResult`（copilot 工具协议）+ `AiGenerationParams` / `AiThinkingConfig` / `AiReasoningEffort` / `AiThinkingMode`（生成参数）+ `AiError`（错误枚举）（RFC-0005，HTTP 调用已前移到前端）
...
Ring 0 **不做**：具体节点实现、脚本引擎、协议驱动（HTTP / MQTT / Modbus / SQL）、IPC 契约类型（由 `tauri-bindings` 承担，见 ADR-0017）、AI HTTP 调用（已前移到前端，RFC-0005）。
...
├── ai.rs               # AiToolCall/AiToolDefinition/AiToolResult + AiGenerationParams + AiError（RFC-0005）
...
- `AiToolCall` / `AiToolDefinition` / `AiToolResult` + `AiGenerationParams` + `AiError` — `src/ai.rs`（copilot 工具协议 + 生成参数，RFC-0005）
...
- **ADR-0019** AI 能力依赖反转（已由 RFC-0005 取代——AI HTTP 调用全部前移到前端）
