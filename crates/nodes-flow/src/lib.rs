...
impl Plugin for FlowPlugin {
    ...
    fn register(&self, registry: &mut NodeRegistry) {
        registry.register("rhai", |def, _res| {
            ...
            Ok(Arc::new(RhaiNode::new(def.id.clone(), config, description)?))
        });
        ...
        registry.register("switch", |def, _res| {
            ...
            Ok(Arc::new(SwitchNode::new(def.id.clone(), config, description)?))
        });
        ...
        registry.register("tryCatch", |def, _res| {
            ...
            Ok(Arc::new(TryCatchNode::new(def.id.clone(), config, description)?))
        });
        ...
        registry.register("loop", |def, _res| {
            ...
            Ok(Arc::new(LoopNode::new(def.id.clone(), config, description)?))
        });
    }
}
