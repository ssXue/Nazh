...
async fn serial_trigger_node_normalizes_ascii_and_hex_frames() {
    ...
    // 本测试只走 transform 路径——传 connection_id=None + 默认 ConnectionManager
    // 即可，不会触发 on_deploy 中的连接借出。
    ...
    match result {
        Ok(execution) => match execution.first() {
            Some(first_output) => {
                ...
                assert_eq!(
                    first_output.metadata.as_ref().unwrap()["serial"]["node_id"],
                    json!("scan_input")
                );
                assert_eq!(
    ...
    match result {
        Ok(execution) => match execution.first() {
            Some(first_output) => {
                ...
                assert_eq!(
                    first_output.metadata.as_ref().unwrap()["http"]["body_mode"],
...
async fn sql_writer_node_persists_payload_into_sqlite() {
    ...
    match result {
        Ok(execution) => match execution.first() {
            Some(first_output) => {
                assert_eq!(
                    first_output.metadata.as_ref().unwrap()["sql_writer"]["table"],
                    json!("workflow_logs")
                );
            }
    ...
    match result {
        Ok(execution) => match execution.first() {
            Some(first_output) => {
                assert_eq!(first_output.payload["status"], json!("ok"));
                assert_eq!(
                    first_output.metadata.as_ref().unwrap()["debug_console"]["label"],
                    json!("tap")
                );
                assert_eq!(
                    first_output.metadata.as_ref().unwrap()["debug_console"]["pretty"],
                    json!(false)
                );
            }
