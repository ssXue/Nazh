//! RFC-0004 Phase 1 集成测试：`DeviceSpec` 解析 → Pin 声明映射。
//!
//! 验证完整的设备建模管道：YAML 文本解析为 `DeviceSpec`，信号映射为 `PinDefinition`，
//! 设备资产文件持久化由 Tauri 壳层工作路径 YAML 命令负责。
...
#[test]
fn 设备_yaml_round_trip_保持结构() {
    let spec = parse_device_yaml(SAMPLE_DEVICE_YAML).expect("YAML 解析应成功");
    let yaml = serde_yaml::to_string(&spec).expect("YAML 序列化应成功");
    let reparsed = parse_device_yaml(&yaml).expect("重新解析应成功");

    assert_eq!(reparsed.id, spec.id);
    assert_eq!(reparsed.device_type, spec.device_type);
    assert_eq!(reparsed.signals.len(), spec.signals.len());
}
