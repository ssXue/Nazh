> **Status:** merged in e0bfbeb

# AI 配置与 API 集成实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为 Nazh 新建 `ai` crate，实现 AI 提供商配置管理和 OpenAI 兼容 API 客户端，在 Tauri 壳层暴露 4 个 IPC 命令，前端设置面板增加 AI 配置区域。

**Architecture:** 三层分离 — `ai`（纯 AI HTTP 客户端 + 配置模型）作为公共层，`src-tauri`（配置时 Copilot IPC）和未来的 `nazh-engine`（运行时 AI 节点）各自依赖它。配置存储在 `app_local_data_dir/ai-config.json`，全局应用级别，不跟随工作区。密钥三视图模型确保前端永远不接触明文 API Key。
...
|--------|------|---------------|
| Create | `crates/ai/Cargo.toml` | crate 元数据和依赖声明 |
| Create | `crates/ai/src/lib.rs` | crate 入口，模块声明，重导出 |
| Create | `crates/ai/src/error.rs` | AiError 独立错误类型 |
| Create | `crates/ai/src/types.rs` | AiCompletionRequest/Response, AiMessage 等 |
| Create | `crates/ai/src/config.rs` | AiConfigFile/View/Update, AiProviderSecretRecord/View/Upsert, AiSecretInput, AiGenerationParams |
| Create | `crates/ai/src/service.rs` | AiService trait 定义 |
| Create | `crates/ai/src/client.rs` | OpenAiCompatibleService 实现 |
| Modify | `Cargo.toml` | workspace members 新增 `"crates/ai"` |
| Modify | `crates/nazh-core/src/error.rs` | 新增 AiNodeError 变体 |
| Modify | `src-tauri/Cargo.toml` | 新增 `ai` 依赖 |
...
### Task 1: 创建 `ai` crate 骨架

**Files:**
- Create: `crates/ai/Cargo.toml`
- Create: `crates/ai/src/lib.rs`
- Create: `crates/ai/src/error.rs`
- Modify: `Cargo.toml` (workspace root)

- [ ] **Step 1: 创建 crate 目录结构**

```bash
mkdir -p crates/ai/src
```

- [ ] **Step 2: 编写 `crates/ai/Cargo.toml`**
...
- [ ] **Step 3: 编写 `crates/ai/src/error.rs`**
...
- [ ] **Step 4: 编写 `crates/ai/src/lib.rs`**
...
```toml
members = [".", "crates/nazh-core", "crates/ai", "src-tauri"]
```

- [ ] **Step 7: 验证编译通过**

Run: `cargo check --package ai`
Expected: 编译成功，无错误
...
git commit -s -m "feat: 新建 ai crate 骨架与 AiError 类型"
```
...
**Files:**
- Modify: `crates/ai/src/config.rs`
...
在 `crates/ai/src/config.rs` 末尾追加：
...
**Files:**
- Modify: `crates/ai/src/service.rs`
