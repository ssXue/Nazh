...
- [x] **Step 1: ADR-0009 状态升级**

Front-matter `状态: 提议中` → `状态: 已实施`。在 `## 备注` 末尾追加一行实施日期与对应 commit。

- [x] **Step 2: docs/adr/README.md 索引更新**
...
- [x] **Step 4: 核对 `#[allow(clippy::too_many_lines)]` 收支表**
...
- [x] `feat(core): 引入 NodeTrait::on_deploy 与 LifecycleGuard 骨架`（Task 0）
- [x] `refactor(graph): 部署路径加入 on_deploy 阶段与逆序撤销`（Task 1）
- [x] `refactor(nodes-io): timer 节点持有自身生命周期`（Task 2）
- [x] `refactor(nodes-io): serial 节点持有自身生命周期`（Task 3）
- [x] `refactor(nodes-io): mqttClient 订阅模式持有自身生命周期`（Task 4）
- [x] `refactor(tauri): 删除 src-tauri 中 timer/serial/mqtt 代持代码`（Task 5）
- [x] `docs: ADR-0009 标记为已实施 + 同步 AGENTS.md 与 README`（Task 6）
