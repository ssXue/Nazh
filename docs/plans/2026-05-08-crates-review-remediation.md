...
> **状态：** 2026-05-09 修复收口；除 CR-P3-09 作为持续治理项不做无目标大重构外，其余问题已完成代码修复与验证
...
> **状态：** 2026-05-09 已修复。`dsl-compiler` 保存 action instance 的来源与 args，不再按 `target_id` 回查首个匹配调用；重复 capability target + 不同 args 已有回归覆盖。
...
> **状态：** 2026-05-09 已修复。Workflow DSL 条件表达式改为编译期校验作用域，裸变量不再静默进入运行时；stateMachine 运行时表达式求值与文档约束同步。
...
> **状态：** 2026-05-09 已修复。未实现的 state timeout / on_timeout 路径先在 DSL 编译期 fail-fast，避免生成看似有保护但运行时不会触发的状态机。
...
> **状态：** 2026-05-09 已修复。未接入执行器的 system action 不再生成伪 `code` 节点，而是在编译期返回可定位错误。
...
> **状态：** 2026-05-09 已修复。`dsl-core` 补 validated parse 路径与 capability/device 校验，模板变量、重复 ID、fallback 引用、required/range 等无效资产会在保存/编译前失败。
...
> **状态：** 2026-05-09 已修复。自动生成 capability 会保留可无损表达的信号语义；对 Modbus/CAN 等无法无损编码的输出信号返回诊断，不再生成看似可执行的能力。
...
> **状态：** 2026-05-09 已修复。Safety 检查改为从 initial 做可达分析，正常状态环不再误报；编译失败会保留完整 `SafetyDiagnostic`。
...
> **状态：** 2026-05-09 已修复。编译上下文拒绝重复 device/capability id，输出节点 ID sanitize 碰撞会在编译期报错并列出冲突来源。
...
> **状态：** 2026-05-09 已修复。连接定义 validator 对未知 `type` 默认拒绝，并在错误中列出支持的连接类型。
...
> **状态：** 2026-05-09 已修复。SLCAN 同步 open/write/flush 移入阻塞隔离路径，CAN shared session 不再把正常读写竞争误报为配置错误。
...
> **状态：** 2026-05-09 已修复。EtherCAT PDO 运行时错误会标记连接失败并清理失效 shared session，避免继续复用坏会话。
...
> **状态：** 2026-05-09 已修复。SQL/CAN/EtherCAT/Modbus 等现场相关配置改为显式声明；测试和 demo 场景通过 `simulation: true` 或显式路径表达，不再靠业务代码静默默认。
...
> **状态：** 2026-05-09 已修复。`store` 新增异步 `StoreHandle` worker 边界，Tauri async 调用方改走 handle，SQLite 同步调用集中在 worker 内串行执行。
...
> **状态：** 2026-05-09 已修复。migration bootstrap 只接受表不存在场景，其它 SQLite 错误直接返回；初始化路径不再用 `expect` panic。
...
> **状态：** 2026-05-09 已修复。`NodeHandle::emit`、`WorkflowIngress::submit` / `submit_to` / `blocking_submit_to`、runner downstream 发送失败均补偿释放 DataStore 引用；多根 `submit` 语义固定为 partial accepted 并有回归覆盖。
...
> **状态：** 2026-05-09 已修复。部署期主拓扑改为只使用 Exec + Reactive 边；Data 边只参与 pull 索引与非 Exec 环检测，并新增 Data-only 边不影响 root 识别的回归测试。
...
> **状态：** 2026-05-09 已修复。脚本 AI helper 的阻塞边界收紧，超时与并发行为由脚本运行层统一治理；相关文档同步当前 helper 注入边界。
...
> **状态：** 2026-05-09 已修复。stateMachine transition 条件求值使用受限 Rhai engine，并应用 `max_operations`，超步数会返回错误。
...
> **状态：** 2026-05-09 已修复。lookup key 类型错误现在携带实际节点 ID，多个 lookup 节点并存时可定位到具体节点。
...
> **状态：** 2026-05-09 已修复。AI 配置拒绝常见敏感 header 通过 `extra_headers` 明文保存/回传，view 只暴露非敏感 header。
...
> **状态：** 2026-05-09 已修复。AI 请求在单次读锁内解析 provider、secret 与 agent settings，形成不可变请求快照后再发送。
...
> **状态：** 持续治理项。按本节修复方案，本项不做一次性大重构；已拆出 `store::handle`、graph topology helper、`connections::types` / `connections::policy` / `connections::validation` / `connections::tests`，并将 `dsl-compiler::safety` 的诊断、模板 helper 与 4 个规则域移入子模块；后续继续随具体修复推进。
...
- `connections/src/lib.rs` 可继续拆：manager/guard/health/shared_session。
- `dsl-compiler/src/safety.rs` 可继续拆：测试外移；生产规则域已拆出。
