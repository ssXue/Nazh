...
| 新建 | `web/src/lib/workflow-node-capabilities.ts` | AI 编排读取节点定义 / 默认 config 键 / 运行时能力 / pin schema 的统一目录 |
| 新建 | `web/src/lib/__tests__/flowgram-subgraph.test.ts` | 展平逻辑单元测试 |
| 新建 | `web/src/lib/__tests__/workflow-node-capabilities.test.ts` | AI 节点能力目录单元测试 |
| 改 | `tests/workflow.rs` | 含 passthrough 节点的集成测试 |
| 改 | `docs/adr/0013-子图与宏系统.md` | 状态更新为已实施（子图核心） |
...
- [x] **Step 3: 运行 cargo check 验证编译**

Run: `cargo check -p nodes-flow`
Expected: 编译通过，无错误

- [x] **Step 4: 运行已有测试确认无回归**
...
- [x] **Step 2: 扩展 normalizeNodeKind 函数**
...
- [x] **Step 3: 扩展 getFallbackNodeLabel 函数**
...
- [x] **Step 2: 创建 subgraphInput/index.ts**
...
- [x] **Step 3: 创建 subgraphOutput/index.ts**
...
- [x] **Step 1: 扩展 FLOWGRAM_BUSINESS_NODE_TYPES**
...
- [x] **Step 6: 修改 toNazhWorkflowGraph 调用展平**
...
- [x] **Step 1: 添加含 PassthroughNode 的集成测试**
...
在 `docs/adr/0013-子图与宏系统.md` 中将状态更新为 `已实施（子图核心）`：
...
git commit -s -m "docs(adr-0013): 标记子图核心已实施"
```
