...
- **设备/连接节点边界收口**（2026-05-05，2026-05-09 部分收口）：评审结论见 `docs/specs/2026-05-05-node-architecture-boundary-review.md`。当前连接资源层方向正确，但普通节点库仍直接暴露 `serialTrigger` / `modbusRead` / `canRead` / `canWrite` 等底层协议节点；`capabilityCall` 已成为 DSL 高级动作入口并接入真实协议执行，设备信号读取/事件入口仍待后续设计。
