...
- RFC-0003 Phase 2 / Phase 3 — **已实施**（2026-05-17）：`observability_records` SQLite 索引表接入 `observability/` 模块，事件/审计/告警直接写 Store（JSONL 双写已移除）；`deployment_audit` 表写入 deploy / undeploy 生命周期动作；变量变更 / 删除写入审计记录；部署 ast_hash 版本变更检测；新增 `query_deployment_audit` IPC（82 命令）。批量 writer 仍待后续。
