...
- ADR-0016（边级可观测性）— **已实施**（2026-05-13，精确统计 2026-06-01）。`EdgeTransmitSummary` 类型 + Runner `EdgeWindow` 100ms 定时窗口累计器 + `BackpressureDetected` 背压检测（队列深度 ≥80% 容量时发射，每窗口限频一次）+ ts-rs 导出 + 前端解析 + 边热力图 CSS（`flowgram-line--heat-{1-4}` 渐变着色 + `flowgram-line--backpressure` 红色闪烁）。`avg_queue_depth`（窗口内队列深度均值）+ `total_payload_bytes`（窗口内 payload 序列化字节数累计）已实施；`BackpressurePolicy` 枚举存在但运行时未消费（当前硬编码 `Block`，策略执行为下一步）。
...
- ~~**ADR-0016 deferred items**（2026-04-30）~~ **已偿还**（2026-05-13，精确统计 2026-06-01）。`BackpressureDetected` 发射逻辑 + 100ms 定时窗口 flush + 前端边热力图已实施。`avg_queue_depth`（窗口内队列深度均值）+ `total_payload_bytes`（payload 字节数累计）已实施。`BackpressurePolicy` 策略执行（枚举存在但运行时未消费）为下一步。
