...
- **[SECURITY] AI API key 明文存储**（2026-05-16 登记）：API key 以明文存储于 `app_local_data_dir()/ai-config.json`，根 `AGENTS.md` 已更新为准确描述。后续应接入 Tauri secure storage 或 OS keychain，将 `ai-config.json` 中 `api_key` 字段替换为 key 引用。
