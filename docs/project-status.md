...
- ADR-0016（边级可观测性）— **已实施**（2026-05-13，2026-06-08 DropOldest 收口）。`EdgeTransmitSummary` 类型 + Runner `EdgeWindow` 100ms 定时窗口累计器 + `BackpressureDetected` 背压检测（队列深度 ≥80% 容量时发射，每窗口限频一次）+ ts-rs 导出 + 前端解析 + 边热力图 CSS（`flowgram-line--heat-{1-4}` 渐变着色 + `flowgram-line--backpressure` 红色闪烁）。`avg_queue_depth`（窗口内队列深度均值）+ `total_payload_bytes`（窗口内 payload 序列化字节数累计）已实施。`BackpressurePolicy` 策略执行已落地（2026-05-13）：Runner 按 Block/DropNewest/Sample/Overflow 四种策略分发。`DropOldest` 变体因 tokio mpsc sender 端无法安全实现已于 2026-06-08 删除。
...
- ~~**ADR-0016 deferred items**（2026-04-30）~~ **已偿还**（2026-05-13 策略执行落地，2026-06-08 DropOldest 收口）。`BackpressureDetected` 发射逻辑 + 100ms 定时窗口 flush + 前端边热力图已实施。`BackpressurePolicy` 策略执行已落地：Runner 按 Block（await send）与非 Block（try_send + drop）分支分发，`EdgeWindow` 维护 `dropped_in_window` 计数器。`DropOldest` 变体因 tokio mpsc sender 端无法安全实现真正的"丢弃最老消息"（sender 无 recv 方法）已删除（2026-06-08），保留 Block / DropNewest / Sample / Overflow 四种策略。
