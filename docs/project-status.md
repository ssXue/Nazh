...
- **设备/连接节点边界收口**（2026-05-05，2026-05-09 部分收口，2026-05-13 完成阶段 2 前端分类调整）：评审结论见 `docs/specs/2026-05-05-node-architecture-boundary-review.md`。当前连接资源层方向正确；`capabilityCall` 已成为 DSL 高级动作入口并接入真实协议执行；前端节点库新增"设备能力"分组并把 capabilityCall 列为业务编排首选，`serialTrigger` / `modbusRead` / `canRead` / `canWrite` / `mqttClient` 文案降级为调试/适配器，capabilityCall 连接绑定 UI 补齐，AI copilot system prompt 同步调整。EtherCAT 三件套维持原状。设备信号读取/事件入口（`deviceSignalRead` / `deviceEventTrigger`）仍待阶段 3 ADR；CAN 模拟回退收紧待阶段 1（后续）。
