...
- [ ] `src/graph/` 总行数超过 **1500 行**（当前 ~510 行，即 3 倍增长后重评）
- [ ] 入参 ADR 之一落地且涉及 `deploy.rs` / `runner.rs` 的**重大改造**（ADR-0014 边分离、ADR-0013 子图、ADR-0009 生命周期钩子同时落地）
