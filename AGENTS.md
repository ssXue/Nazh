...
- ADR-0013（子图与宏系统）— **前端层已 merge，deployment 链路断**（2026-04-28 审计发现）。前端 FlowGram `subgraph` 容器 + `subgraphInput` / `subgraphOutput` 桥接节点 + AI 编排器扩展已落地；但三处部署断链让任何含子图的工作流目前无法部署：(a) `web/src/lib/__tests__/flowgram-subgraph.test.ts` 引用的 `flattenSubgraphs` 函数在仓库中无定义；(b) `crates/nodes-flow/src/passthrough.rs` 是孤儿源文件，`crates/nodes-flow/src/lib.rs` 既未声明 `mod passthrough` 也未在 `FlowPlugin::register` 中注册；(c) `web/src/lib/flowgram.ts` 的 `toNazhWorkflowGraph` 直接过滤掉 subgraph 类型节点，其内部 `blocks` 也跟着被丢——原设计意图是先调 flattenSubgraphs 再过滤，但 flatten 缺失。详见 memory `project_subgraph_deployment_broken.md`。在三处断链补完前，AGENTS.md 维持 ADR-0013 = proposed 而非"已实施"。
- ADR-0015 / ADR-0016 / ADR-0020 — **proposed**, awaiting review. See `docs/adr/README.md` for the index.

**Immediate known tech debt:**
- **ADR-0013 子图实施 deployment 断链**（2026-04-28 发现）：前端拖入子图后无法实际部署。涉及 `flattenSubgraphs` 缺实现 + `passthrough` Rust 节点未注册 + `toNazhWorkflowGraph` 过滤顺序错位。详见 memory `project_subgraph_deployment_broken.md`；ssXue 是 ADR-0013 实施 owner，建议由其修通后再标"已实施"。**ADR-0014 Phase 3 PURE 节点的 plan 应以子图修通为前置条件**——否则 PURE 节点 + 子图组合的 PinKind 边界处理无意义。
